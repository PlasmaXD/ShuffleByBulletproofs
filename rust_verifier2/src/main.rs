...
fn shuffle_commitments(input_file: &str) {
    ...
    let mut original_commitments = input.commitments.iter().map(|hex| {
        ...
        CompressedRistretto::from_slice(&bytes).unwrap()
    }).collect::<Vec<_>>();
...
fn verify_shuffle(input_file: &str) {
    ...
    let valid = match proof_type.as_str() {
        ...
        _ => {
            ...
            let proof_before = CompressedRistretto::from_slice(&pb).unwrap().decompress().unwrap();
            let proof_after  = CompressedRistretto::from_slice(&pa).unwrap().decompress().unwrap();
